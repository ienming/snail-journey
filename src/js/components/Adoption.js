    ...
    template: `
        <div class="popup scroll adopt">
            <div class="adopt-goal">
                <h3 class="mb-1">Goal of the project</h3>
            ...
            <basic-link class="mt-1" title="Details of the project"></basic-link>
        ...
        adopt(){
            ...
            if (vm.$data.user.adoptions.every(val => val !== this.program.title)){
                if (this.program.subTitle){
                    //打事件到vm 呼叫 showsys()
                    this.$emit(`adopted`, `Participate in "${this.program.subTitle}：${this.program.title}"`)
                    this.getReward()
                }
                else{
                    this.$emit(`adopted`, `Participate in "${this.program.title}"`)
                    this.getReward()
                }
                //把認養的資料儲存
                vm.$data.user.adoptions.push(this.program.title)
            }else{
                this.$emit(`adopted`, `You have already take part in "${this.program.title}"～`)
            }
        },
        getReward(){
            vm.$data.user.coins += this.program.reward
            let str = `Got the rewards of participating in "${this.program.title}"`
            let imgUrl = "./src/img/coin.png"
            let num = this.program.reward
            let abs = `Got ${num} Snail Coins`
